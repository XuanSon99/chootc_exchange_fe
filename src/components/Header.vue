<template>
  <div>
    <div v-if="!mobile">
      <v-app-bar color="#ffffff" height="68px" fixed>
        <v-container class="d-flex align-center">
          ...
          <v-btn class="ml-4" color="primary" @click="$router.push('/login')" v-if="!account">Đăng
            nhập</v-btn>
          <v-btn class="ml-4" color="primary" outlined @click="$router.push('/register')" v-if="!account">Đăng
            ký</v-btn>
        </v-container>
    ...
    <div v-else>
      <v-bottom-navigation v-model="mbar" color="primary" fixed shift grow>
        <v-btn to="/home" active-class="">
          <span>Trang chủ</span>
          <v-icon>mdi-home</v-icon>
        </v-btn>

        <v-btn to="/history/buy" active-class="">
          <span>Lịch sử</span>
          <v-icon>mdi-history</v-icon>
        </v-btn>

        <v-btn to="/notification" active-class="">
          <span>Thông báo</span>
